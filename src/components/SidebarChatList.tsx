...
const SidebarChatList: FC<SidebarChatListProps> = ({friends , sessionId}) => {
  ...
  useEffect(()=>{
      ...
      const chatHandler = (message:ExtendedMessage)=>{
         ...
         toast.custom((t)=>(
            <UnseenChatToast t={t} senderId={message.senderId} senderImg={message.senderImg} senderMsg={message.text} senderName={message.senderName} sessionId={sessionId}/>
         ))
  ...
  return (<ul role='list' className='max-h-[25rem] overflow-y-auto -mx-2 space-y-1'>
    {activeChats.sort().map((friend)=>{
       ...
       const unseenMessagesCount = unseenMessages.filter((messages)=>{
          return messages.senderId === friend.id;
       }).length;
